...
image: prepare mbr loader kernel disk
	rm -rf disk.img && bximage -func=create -hd=10 -imgmode=flat -sectsize=4096 -q disk.img 1>/dev/null
	dd if=$(BIN_DIR)/mbr of=disk.img bs=512 count=1 seek=0 conv=notrunc
	dd if=$(BIN_DIR)/loader of=disk.img bs=512 count=8 seek=1 conv=notrunc
	dd if=$(BIN_DIR)/kernel of=disk.img bs=512 count=2048 seek=9 conv=notrunc
	dd if=$(USER_DIR)/user_disk_image of=disk.img bs=512 count=2048 seek=2057 conv=notrunc
...
clean:
	rm -rf ${OBJ_DIR}/* ${BIN_DIR}/* dsik.img* bochsout.txt kernel_dump.txt
